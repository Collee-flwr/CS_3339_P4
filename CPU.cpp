...
void CPU::printFinalStats() {
 cout << "Program finished at pc = 0x" << hex << pc << "  ("
  ...
  cout << "Cycles: " << stats.getCycles() << endl
       ...
       << "RAW hazards: " << stats.getTotalHazards()
       << " (1 per every " << instructions/ (float)stats.getTotalHazards() << " instructions)" << endl
       << "   On EXE1 op: " << stats.getExe1Hazards() << setprecision(0) << " (" << 100 * (float)stats.getExe1Hazards()/ stats.getTotalHazards() << "%)" << endl
       << "   On EXE2 op: " << stats.getExe2Hazards() << " (" << 100 * (float)stats.getExe2Hazards()/ stats.getTotalHazards() << "%)" << endl
       << "   On MEM1 op: " << stats.getMem1Hazards() <<  " (" << 100 * (float)stats.getMem1Hazards()/ stats.getTotalHazards() << "%)" << endl
       << "   On MEM2 op: " << stats.getMem2Hazards() << " (" << 100 * (float)stats.getMem2Hazards()/ stats.getTotalHazards() << "%)" << endl << endl;
